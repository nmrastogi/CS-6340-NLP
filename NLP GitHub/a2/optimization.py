...
OPTIMAL_STEP_SIZE = 0.1  # Adjust this value after experimenting with the learning rate
###
...
def quadratic_grad(x1, x2):
    """
    Returns the gradient of the quadratic function.
    :param x1: first coordinate
    :param x2: second coordinate
...
def sgd_test_quadratic(args):
    ...
    points_history = []
    curr_point = np.array([0., 0.])  # Starting at the origin
    for iter in range(0, args.epochs):
        ...
        # Stop early if within a distance of 0.1 from the optimal point (1, 1)
        if np.linalg.norm(curr_point - np.array([1., 1.])) < 0.1:
            print("Converged after %i epochs." % (iter + 1))
            break
